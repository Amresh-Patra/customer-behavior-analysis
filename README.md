# 🧠 Customer Behavior Analysis — SQL Portfolio

SQL project analyzing customer order patterns, revenue trends,
and behavior using real business problems.

---

## 👤 About
**Name:** Amresh
**Goal:** Job-ready SQL and Data Analyst skills
**Focus:** Real business problems — not just syntax

---

## 📌 Problems Solved

| # | Problem | Concepts Used |
|---|---------|---------------|
| 1 | Consecutive Orders | LAG, DATEDIFF |
| 2 | Top 3 Customers by Revenue | GROUP BY, SUM, ORDER BY, LIMIT |
| 3 | Customer Ranking by Revenue | DENSE_RANK, GROUP BY, Subquery |
| 4 | Top Customer Per City | ROW_NUMBER, PARTITION BY, GROUP BY |
| 5 | Customer Segmentation | CASE WHEN, GROUP BY, SUM |
---

## 🧠 Skills Covered

- Window Functions — LAG, LEAD, RANK, DENSE_RANK, ROW_NUMBER
- PARTITION BY and ORDER BY
- Subqueries and Correlated Subqueries
- EXISTS / NOT EXISTS
- GROUP BY, HAVING, Aggregations
- Streak and Consecutive Logic
- Top N / Bottom N patterns
- CASE WHEN conditional logic
- CTEs — Common Table Expressions

---

## 🎯 Goal

To become job-ready for SQL and data analyst roles by solving
real business problems — customer retention, revenue trends,
segmentation, and churn analysis.

---

## 📈 Progress

### ✅ Completed
- [x] Consecutive Orders — LAG + DATEDIFF
- [x] Top 3 Customers by Revenue — GROUP BY, SUM, LIMIT
- [x] Customer Ranking — DENSE_RANK
- [x] Top Customer Per City — ROW_NUMBER, PARTITION BY
- [x] Customer Segmentation — CASE WHEN

### 🔶 In Progress
- [ ] CTEs

### 🔒 Coming Soon
- [ ] Streak Analysis
- [ ] Monthly Revenue Trends
- [ ] Churn / Inactive Customer Analysis
- [ ] Customer Retention Patterns
